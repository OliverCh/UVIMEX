...
function cambiarDiap(diapositiva){
	...
	$.getJSON("curso.json",function(data){
		...
		jQuery.get(data.contenido[diapositiva].template  +'.txt', function(datos) {
			...
			if(data.contenido[diapositiva].template == "actInversion"){
				cargarPortafolio();
			}
 ...
 setTimeout('buscarImagen($("#controlVideo").val(),tiempos)', 200);
	 setInterval('actualizaBarraVideo();',500);
}
//**************************************************************************
...
function cargarActividad(preguntas, template){
	...
	if(tipoActividad == "0" || tipoActividad == "1" || tipoActividad == "3" || tipoActividad == "4" || tipoActividad == "5" || tipoActividad == "6" || temp == "actInversion"){
		respuestas = [];
		resps6 = [];
	}
	...
	if(respuestas != null){
		...
		if(tipoActividad == "2"){
			...
			for(var i in respsBien){
          	...
          	$('<button id="imprimirActividad">Imprimir</button>').prependTo("#contenedor");
...
function calcahorro(){
	var gasto = parseInt($("#monto").val());
	var c = gasto * 6;
	var v = gasto * 3;
	$("#ahorro").text("Tu ahorro debe estar entre: $" + v + " y $" + c);
}

function cargaQuizInv(){
	console.log("A");
	$("#contenedorCurso").append(`
		<div id="invQuiz">
			<a href="https://storage.googleapis.com/denlin-cursos/Moris/C4/Quiz.%20Perfil%20de%20inversionista.docx">Descargar Quiz perfil de inversionista</a>
			<select id="perfilInv" onChange="saveInv();">
				<option disabled selected>Selecciona tu tipo de inversionista</option>
				<option value="Muy conservador">Muy conservador</option>
...
function saveInv(){
	localStorage.setItem("perfilInv",$("#perfilInv").val());
	alert("Guardado");
}
...
var portafolioItem = `
<div id="item_:idx:" class="portafolioItem">
	<p>:nombre: | :p:%</p>
	<button onclick="delPorta(:idx:);">ELIMINAR</button>
</div>
`;
...
function cargarPortafolio(){
	...
	if(portafolio == null){
		...
		var _totalPorta = 0;
		for(var x in portafolio){
			if(portafolio[x] != null){
				_totalPorta += parseInt(portafolio[x]);
				$("#portafolio").append(portafolioItem.replace(":nombre:", getIns(x)).replace(":p:", parseInt(portafolio[x])).replace(":idx:", x).replace(":idx:",x));
			}
		}

		$("#totalPortafolio").text("Total: " + _totalPorta + "%");
	}
...
function guardarPortafolio(){
	...
	if($("#porcentajeP").val() == "")
		return false;
	if(portafolio[$("select").val()]){
		$("#item_"+ $("select").val()).remove();
	}
	portafolio[$("select").val()] =  parseInt($("#porcentajeP").val());
	$("#portafolio").append(portafolioItem.replace(":nombre:", getIns($("select").val())).replace(":p:", parseInt($("#porcentajeP").val())).replace(":idx:", $("select").val()).replace(":idx:", $("select").val()));
	$("#porcentajeP").val("");
	localStorage.setItem("portafolio", JSON.stringify(portafolio));
	var tot = TotalPorta();
	$("#totalPortafolio").text("Total: "+ tot + "%");
}

function validarTot(changeTot){
	var tot = TotalPorta();
	if(portafolio[$("select").val()]){
		tot -= portafolio[$("select").val()];
	}
	console.log(portafolio[$("select").val()]);
	if(((tot + parseInt($("#porcentajeP").val())) > 100)){
		alert("No puedes agregar mas del 100% de tu portafolio");
		$("#porcentajeP").val("");
	}
	else{
		if(changeTot == false){
...
function TotalPorta(){
	...
	for(var i in portafolio){
		if(portafolio[i] != null){
			tot += parseInt(portafolio[i]);
		}
	}
	return tot;
}

function delPorta(id){
	portafolio = JSON.parse(localStorage.getItem("portafolio"));
