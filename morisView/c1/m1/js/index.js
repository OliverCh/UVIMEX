...
function cargarActividad(preguntas, template){
	...
	if(tipoActividad == "0" || tipoActividad == "1"){
		respuestas = [];
	}
...
function terminarAct(){
	if(respuestas != null){
		...
		if(tipoActividad == "2"){
			...
			console.log(eval);
			for(var i in eval){
				console.log(respuestas > parseInt(eval[i][0]), respuestas, eval[i][0], Object.size(eval), i +1);
				if(parseInt(i) == Object.size(eval) - 1){
					...
					if((respuestas <= parseInt(eval[i][0])) && (respuestas > parseInt(eval[parseInt(i) + 1][0]))){
						resultadoHTML += '<p class="evalItem evalItem_selected">'+eval[i][1]+'</p>';
					}
					else{
						resultadoHTML += '<p class="evalItem">'+eval[i][1]+'</p>';
...
$("#diapSig").click(function(){
	...
	////console.log(numDiap);
	if (numDiap == totalDiap ) { cargaMenu(); }else{numDiap ++;}
...
//**************************************************************************
    ...
    function actualizaBarraVideo (){
		...
		if(document.getElementById("videoCurso")){
