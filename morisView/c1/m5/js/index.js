...
function cambiarDiap(diapositiva){
	...
	$.getJSON("curso.json",function(data){
		...
		jQuery.get(data.contenido[diapositiva].template  +'.txt', function(datos) {
			...
			if(data.contenido[diapositiva].template == "act4")
				respuestas = [];
...
function calcdeuda(){
	var ingreso = parseInt($("#ingreso").val());
	var gasto = parseInt($("#gasto").val());
	var c = (ingreso - gasto) * .3;
	$("#endeudamiento").text("Tu capacidad de endeudamiento es de $" + c);
}
