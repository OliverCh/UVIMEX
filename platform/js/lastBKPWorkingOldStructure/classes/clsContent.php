...
class Content extends dbHandler{
  ...
  //Load Single Content (Select)
	...
	{
		$strSQLoadContent= $this->selectQuery("select idContenido,idTema,contenido,video,audio,titulo,subtitulo,orden from contenido where idTema={$idTema}");
        if($strSQLoadContent){
	...
	{
    $strSQLSaveContent= $this->prepare("INSERT INTO contenido (idTema, contenido,video,audio,titulo,subtitulo,orden)
                                                       VALUES (:idTema, :contenido, :video, :audio, :titulo, :subtitulo, :orden)");
