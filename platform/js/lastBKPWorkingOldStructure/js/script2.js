...
function plantillaSelect(id){
  ...
  var texto = $('.den-curso-title.den-select-option p').text();
   jQuery.get( callingTxt, function(datos) {
    $('.den-prevnedit-plantilla').html(datos);
        cargarVideoAudio('video1','video','video');
...
function createNewTitle(idTema){
  function buscarTexto(idTema){
    ...
    if(buscarCoincidencias.length === 0){
      ...
      width = width*number;
      size = size * 150;
      // Asignar id padre-hijo
      var idPantalla = 'Pantalla_'+idTema;
      var idPTitle = 'titlePantalla_'+idTema;
      //Remover Select class a losotros  y eliminar contenidos
      $('.den-curso-title').removeClass('den-select-option');
      $('.den-pantalla-space').remove();
      //Crear Nuevos Conenidos
      ...
      //Ocultar el Pop donde ingresaste el nuevo tema
      $('.den-popup-newtitle').addClass('den-hide');
      $('.den-popup-newtitle input').val('');
      // Encontrar el id de la plantilla que hay que inyectar
      var id = $('.den-plantilla-title.den-select-option').attr('id');
      plantillaSelect(id);
      //Animacion de El carrusel
      ...
      cargarAlert(tipo,valor);
    }
  }
  var saveNewTxt = $('.den-popup-newtitle input').val();
  if (saveNewTxt === '') {
    $('.den-popup-newtitle input').val('');
    cargarAlert(tipo,valor);
    }else{
...
function goToOtherTitle(){
  ...
  //
  $('.den-curso-title.den-select-option').removeClass('den-go-title');
...
<div class='base-herramientas-video base-display-hidden'>\
  ...
  <div class='base-fileplay tooltip'>
  <i class="fas fa-play-circle"></i><span class='tooltiptext'>Reproducir</span>
  </div>
</div>\
<input type='file' accept='video/*' type='file ' class='form-control-file' onchange='readVideoUrl(this)'><div class='base-nameoffile'></div></div>`;

var audioFileContainer = `<div class='base-agrega-material base-agregar-video base-thishasfile addFile-input-onclick' style='border-color:#8ad3a0;'><span class='base-plus-span'></span>\
...
<input type='file' accept='audio/*' type='file ' class='form-control-file' onchange='readVideoUrl(this)' style='display:none;'><div class='base-nameoffile'></div>\
<input type='number' class='contador-video' value='' style='display:block;'></div>`;
...
//////FUNCTIONS FOR WEB REAL

function cargarTemplate(id,tema,subtema,multimedia) {
   var elementos = $('.den-curso-title');
...
function cargarAlert(tipo,valor){
  $('.base-genericalert').remove();
  $('<div class="base-alert-style base-genericalert"><h1></h1><p></p></div>').appendTo('body')
  var titulo = $('.base-genericalert h1');
  var texto =$('.base-genericalert p');
  if (valor = true) {
    $('base-genericalert').addClass('base-truealert');
    switch (tipo) {
      case 1:
      titulo.text('El archivo se subio de manera correcta.');
      break;
      case 2:
      titulo.text('Tus cambios se guardaron correctamente');
      break;
      case 3:
      titulo.text('El tema fue creado con exito');
      break;
    }
  }else{
    switch (tipo) {
      case 1:
      titulo.text('¡Ya ingresaste un archivo con este nombre!');
      texto.text('Intenta subir el archivo con otro nombre o un archivo diferente.');
      break;
      case 2:
      titulo.text('Estas repitiendo número de Orden');
      break;
      case 3:
      titulo.text('No ingresaste ningun texto');
      break;
      case 4:
      titulo.text('Ya tienes un tema con este nombre');
      break;
    }

  }
  //Animar la alerta
  $('.base-genericalert').fadeIn('fast').animate({top: 450, opacity:1}, function(){
    setTimeout(function(){
         $('.base-alert-extenfile').fadeOut('slow');
      }, 2000);
  });
}
function cargarPagActividad(valorCheck, id,){
  $('.den-pagina-title').removeClass('den-select-option');
  $('.den-actividadid-visible .tipo-de-actividad').remove();
  //apend Pagina al Carrusel
  $("<div class='den-pagina-title den-select-option'><p></p></div>").appendTo(".den-carouselpaginas-container.den-cp-visible");
  //apend Pagina al Container
  ...
  var elementos = $('.den-cp-visible .den-pagina-title');
  elementos = elementos.size()+1;
  var saveNewTxt = 'Pagina '+elementos;
  var saveNewId = 'actividadPagina_'+elementos;
  var callingTxt = id+'.txt';
  $('.den-actividadid-visible .pagina-actividad-visible').attr('id', saveNewId );
  $('.den-cp-visible .den-pagina-title.den-select-option p').text(saveNewTxt);
  jQuery.get('actividades/'+callingTxt).success(function (datos) {
...
//
function cargarVideoAudio(inputName, inputAccept, tipo) {
$('.base-agrega-material').removeClass('addFile-input-onclick');
$('.base-agregar-video').css('border-color','#ede8e8');
...
<input type='file' accept='video/*' type='file ' class='form-control-file' onchange='readVideoUrl(this)' style='display:none;'><div class='base-nameoffile'>`+ inputName+`</div>
<input type='number' class='contador-video' value='' style='display:block;'></div>`;
var icoFile;
   ///// haciendo Distinción entre Audio y Video PADRES
   switch (inputAccept) {
     ...
     $(fileContainer).prependTo('.base-only-video');
     break;
     case 'video':
...
callChildren(children,inputName);
}
function callChildren(children,inputName){
  switch (children) {
    case 'setimages':
    $('.base-imagesforvideo-container').remove();
    $("<div class='base-imagesforvideo-container actual-set-of-images' id=''>"+singleImageContainer+"</div>").appendTo(".den-prevnedit-plantilla .base-only-images");
    $('.den-prevnedit-plantilla .actual-set-of-images').attr('id', inputName);
    break;
    case 'setactividad':
    $('.base-actividad-container').remove();
    $("<div class='base-actividad-container actividad-visible' id=''>"+singleActivContainer+"</div>").appendTo(".base-agregar-actividad");
    $('.den-prevnedit-plantilla .actividad-visible').attr('id', inputName);
    break;
    case 'setparrafo':
    $('base-agregar-parrafo').remove();
    $("<div class='base-agregar-parrafo cop_active'></div>" ).appendTo('.den-prevnedit-plantilla .containerOfPagragraph');
    var setofParagraphs = inputName+'_txt';
    $('.den-prevnedit-plantilla .cop_active').attr('id',setofParagraphs);
  }
}
