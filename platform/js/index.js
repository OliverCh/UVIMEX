...
var index = function() {
	...
	function initComponents(){
		...
		<form><input type='file' accept='image/*' onchange='readImageUrl(this)' class='form-control-file'>\
		</form>
		...
		<form><input type='file' accept='video/*' type='file ' class='form-control-file' id='nuevoMultimedia' onchange='readVideoUrl(this)'>\
		</form><div class='base-nameoffile'></div></div>`;
		...
		$(document).on('click', '.closePopUp', function() {
			$('.den-popup-newtitle').hide(300);
			$('.den-popup-estilo-larger').hide(300);
			$('.den-popup-cambiardetitle').hide(300);
		...
		//Cerrar popUps
		$(document).on('click', '.closethisPop', function() {
			var padre = $(this).parent();
			$(padre).addClass('.den-hide');
		});
		...
		$("#generarNuevaPantallaBtn").click(function(){
			...
			$.ajax({
				...
				success: function(result){
					//quita seleccionado
					$('.den-popup-newtitle').hide(300);
					$('.den-carousel-container .den-select-option').removeClass('den-select-option');
					$("<div class='den-curso-title den-select-option' onclick='chooseTheme(this);'><p>"+saveNewTxt+"</p></div>").prependTo(".den-carousel-container");
				}
			});
		});
		...
		$(document).on('click', '#titulos_option', function() {
			...
			if(themesArray){
				for (var i = 0; i < themesArray.length; i++) {
					$("<div class='den-curso-title' onclick='chooseTheme(this);' data-idCurso='"+themesArray[i].idCurso+"' data-idTema='"+	themesArray[i].idTema+"'><p>"+themesArray[i].nombre+"</p></div>").appendTo(".den-carousel-container");
				}
			}else{
				$(".den-carousel-container").empty();
			}
	...
	$.ajax({
		...
		success: function(arrayResponse){
			...
			for (var i = 0; i < arrayResponse.length; i++) {
				strSelected= (i==(arrayResponse.length-1)) ? 'selected':'';
...
function reloadStoredThemes(){
	...
	success: function(arrayResponse){
		for (var i = 0; i < arrayResponse.length; i++) {
			$("<div class='den-curso-title' onclick='chooseTheme(this);' data-idCurso='"+arrayResponse[i].idCurso+"' data-idTema='"+	arrayResponse[i].idTema+"'><p>"+arrayResponse[i].nombre+"</p></div>").appendTo(".den-carousel-container");
		}
	}
});
}
...
audio:['mp3','wav'] };
//checar si input vacio llena otro
function checkInputFileVideo(element){
	if(element.files.length !=0){
		var aux=element.value.split('.');
		var fileExt= aux[aux.length-1].toLowerCase();
		if(!(ACCEPTED_FILES.video.includes(fileExt))){
			alert('no se acepta este tipo de archivos');
		}else{
			var consecutivo=parseInt(element.dataset.consecutivo)+1;
			$('#wrapVideos').append('<div class="base-agrega-material base-agregar-video">'+
												    '<div class="base-herramientas-video base-display-hidden">'+
												    ...
												    '</div>'+
												    '<span class="base-plus-span">+</span>'+
												    '<input data-consecutivo="'+consecutivo+'" type="file" accept="video/*" type="file" class="form-control-file" id="video'+consecutivo+'" name="video[]"  onChange="checkInputFileVideo(this);">'+
												    '<div class="base-nameoffile"></div>'+
												'</div>');
		}
	}
}
function checkInputFileImage(element){
	if(element.files.length !=0){
			var aux=element.value.split('.');
			var fileExt= aux[aux.length-1].toLowerCase();
			if(!(ACCEPTED_FILES.images.includes(fileExt))){
			...
			var consecutivo=parseInt(element.dataset.consecutivo)+1;
			$('#wrapImages').append('<div class="base-agrega-material base-agregar-video">'+
												    '<div class="base-herramientas-video base-display-hidden">'+
												        '<div class="base-fileedit tooltip editvideo">'+
												            '<i class="fas fa-edit"></i><span class="tooltiptext">Editar</span>'+
												        '</div>'+
												    ...
												    '</div>'+
												    '<span class="base-plus-span">+</span>'+
												    '<input data-consecutivo="'+consecutivo+'" type="file" accept="image/*" type="file" class="form-control-file" id="video'+consecutivo+'" name="video[]"  onChange="checkInputFileImage(this);">'+
												    '<div class="base-nameoffile"></div>'+
														'<input data-consecutivo="'+consecutivo+'" class="timeImage" type="text" name="timeImage[]" class="input-popup-newtitle" value="" placeholder="Minuto">'+
												'</div>');
		}
	}
}

function checkInputFileAudio(element){
	if(element.files.length !=0){
			var aux=element.value.split('.');
			var fileExt= aux[aux.length-1].toLowerCase();
			if(!(ACCEPTED_FILES.audio.includes(fileExt))){
			...
			var consecutivo=parseInt(element.dataset.consecutivo)+1;
			$('#wrapAudios').append('<div class="base-agrega-material base-agregar-video">'+
												    '<div class="base-herramientas-video base-display-hidden">'+
												        '<div class="base-fileedit tooltip editvideo">'+
												            '<i class="fas fa-edit"></i><span class="tooltiptext">Editar</span>'+
												        '</div>'+
												    ...
												    '</div>'+
												    '<span class="base-plus-span">+</span>'+
												    '<input data-consecutivo="'+consecutivo+'" type="file" accept="audio/*" type="file" class="form-control-file" id="audio'+consecutivo+'" name="audio[]"  onChange="checkInputFileAudio(this);">'+
												    '<div class="base-nameoffile"></div>'+
												'</div>');
		}
	}
...
function chooseTheme(x){
	$(x).siblings(".den-select-option").removeClass('den-select-option');
	$(x).addClass('den-select-option');
	$("#idTema").val($(x).data('idtema'));
}
