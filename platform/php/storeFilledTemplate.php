...
include_once("../classes/clsCourseFiles.php");
  ...
  $con-> beginTransaction();
    ...
    if($_REQUEST['idTema']){
      ...
      //THIS WILL SAVE COURSE STATUS TO ON EDITION
      $selectedCourse=$con->selectQuery("SELECT t3.*
                                          FROM temas t1
                                          JOIN  modulos t2
                                          ON t1.idModulo=t2.idModulo
                                          JOIN cursos t3
                                          ON t2.idCurso=t3.idCurso
                                          WHERE t1.idTema={$_REQUEST['idTema']}
                                          LIMIT 1;");
      $course->set_idCurso($selectedCourse->idCurso);
      $course->set_idStatus(1);
    ...
    if(isset($_REQUEST['temaPadre'])){
     ...
     $theme->set_nombre($_REQUEST['title']);
          ...
          foreach($_REQUEST['belonger'] as $llave => $key){
              ...
              if(isset($_FILES['video']['name'][$key]) && !empty($_FILES['video']['name'][$key])){
                  if($_FILES['video']['name'][$key]!=""){
                      ...
                      $content->set_duracion($_REQUEST['videoDuration'][$key]);
          ...
          foreach($_REQUEST['belongerAudio'] as $llave => $key){
              ...
              if(isset($_FILES['audio']['name'][$key]) && !empty($_FILES['audio']['name'][$key])){
                  if($_FILES['audio']['name'][$key]!=""){
                      ...
                      $content->set_orden($_REQUEST['orden'][$key]);
                      $content->set_duracion($_REQUEST['audioDuration'][$key]);
                      // // print_object($content);
                      if($hidContenido){
                        $content->set_idContenido($hidContenido);
                        $content->pubf_EditContent();
          ...
          $hIdArchivosPorCurso=null;
            if($_FILES['archivo']['name'][$key]!=""){
              ...
              /*Ends delete video*/
                ...
                $courseFile->set_strNombreArchivo(isset($_REQUEST['strNombreArchivo'][$key]) ? $_REQUEST['strNombreArchivo'][$key]: null);
